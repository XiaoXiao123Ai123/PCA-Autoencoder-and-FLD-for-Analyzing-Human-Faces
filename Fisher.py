...
def save_projections(train, lm_train, lm_male, lm_female, male_v_ch, female_v_ch):
    # 这里是保存对应的pkl文件，生成男性male文件时将save_geo_projections(lm_train, lm_female)、save_appear_projections(train, female_v_ch)注释，
    # 生成女性female文件时将save_geo_projections(lm_train, lm_male)、save_appear_projections(train, male_v_ch)注释
    # 注意保存文件时需要将PCA中的reconstruct_landmarks、reconstruct函数生成保存pkl文件的名字进行更改对应，男性的文件名字对应male，女性文件名字对应female，测试文件名字添加test
...
def run_fisher():
    ...
    red_imgs_male = get_red_imgs()  # (330, 60)
    red_imgs_female = get_red_imgs(True)  # (470, 60)
    # red_imgs_male = lm_train_male  # (330, 60)
    # red_imgs_female = lm_train_female  # (470, 60)
    mean_male = calc_mean(red_imgs_male)  # (60,)
    mean_female = calc_mean(red_imgs_female)  # (60,)
    ...
    # # save_appear_projections_after_alignment(train, test_male_v_ch, lm_train)
    # save_appear_projections_after_alignment(train, train_male_v_ch, lm_train)
    # save_appear_projections_after_alignment(train, train_female_v_ch, lm_train)
